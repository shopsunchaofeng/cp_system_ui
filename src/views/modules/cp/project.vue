    ...
    <el-table :data="dataList" border @selection-change="selectionChangeHandle" style="width: 100%;">
      ...
      <el-table-column prop="status" header-align="center" align="center" label="状态">
        <template slot-scope="scope">
          ...
          <el-tag v-if="scope.row.status ===2">测评中</el-tag>
          <el-tag v-if="scope.row.status ===3">单元测评完成</el-tag>
      ...
      <el-table-column fixed="right" header-align="center" align="center" width="150" label="操作">
        <template slot-scope="scope">
          ...
          <!--非测评机构按钮-->
          <el-button v-if="isAuth('cp:project:dycp')&& !showhzFlag && scope.row.status ===2" type="text" size="small" @click="fcpwnapeListHandle(scope.row.id)">单元测评</el-button>
          <!--测评机构按钮-->
          ...
          <el-button v-if="isAuth('cp:nape:bushihe') && scope.row.status ===0 && showhzFlag" type="text" size="small" @click="bushiheHandle(scope.row.id,scope.row.systemdengji)">挑选不适合项</el-button>
          <el-button v-if="isAuth('cp:nape:fenpei') && scope.row.status ===1 && showhzFlag" type="text" size="small" @click="fenpeiHandle(scope.row.id,scope.row.systemdengji)">分配测评任务</el-button>
          <el-button v-if="isAuth('cp:nape:cprw') && scope.row.status ===2 && showhzFlag" type="text" size="small" @click="cprwHandle(scope.row.id,scope.row.systemdengji)">查看测评任务</el-button>
          <el-button v-if="isAuth('cp:project:cpObject') && scope.row.zcpuid !=='0' && showhzFlag" type="text" size="small" @click="editContentListHandle(scope.row.id)"> 编辑测评对象</el-button>
